    ...
    with open('contents.txt', 'r') as f:
        for line in f:
            ...
            if line[0] == '[':
                ...
                subsections.append((filename, subsection_name, folder_name))
    return sections
...
def get_tex(sections):
    ...
    for (section_name, subsections) in sections:
        tex += '\\section{%s}\n' % texify(section_name)
        for (filename, subsection_name, folder_name) in subsections:
            tex += '\\subsection{%s}\n' % texify(subsection_name)
            tex += '\\raggedbottom\\lstinputlisting[style=%s]{%s/%s/%s}\n' % (get_style(filename), code_dir, folder_name, filename)
            tex += '\\hrulefill\n'
        tex += '\n'
    return tex
